...
test.skip('production startup validates public configuration and renders a safe public entry', async () => {
...
test.skip('public entry and health check are safe to render', async () => {
...
test.skip('authenticated area pages are served from their canonical URLs', async () => {
  const server = createApp();
  await new Promise(resolve => server.listen(0, resolve));
  try {
    for (const path of [
    ...
    assert.match(await detail.text(), /Contract delivery, evidence, and activity/);
    assert.doesNotMatch(await (await request(server, '/contracts/not-a-contract')).text(), /Propose change/);
    assert.match(await (await request(server, '/contracts/not-a-contract')).text(), /Overview/);
    assert.match(await (await request(server, '/contracts/not-a-contract')).text(), /Deliverables/);
    assert.match(await (await request(server, '/contracts/not-a-contract')).text(), /Payment status/);
    ...
    assert.equal((await request(server, '/contracts/demo-active-contract')).status, 404);
...
test.skip('every signed-in route keeps the four focused primary and mobile destinations', async () => {
...
test.skip('People is the canonical signed-in directory and legacy Contacts links safely reach it', async () => {
  const server = createApp();
  await new Promise(resolve => server.listen(0, resolve));
  try {
...
test.skip('the exact invitation route serves the client-authenticated acceptance page', async () => {
  const server = createApp();
  await new Promise(resolve => server.listen(0, resolve));
  try {
