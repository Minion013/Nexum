...
test('only the invited Profile can read a Contract invitation state before accepting it', async () => {
  const calls = [];
  ...
  try {
    assert.equal((await request(origin, `/api/invitations/${invitationId}`)).status, 401);
    assert.equal((await request(origin, '/api/invitations/not-an-invitation', { token: 'invited-profile-jwt' })).status, 422);
    const invitation = await request(origin, `/api/invitations/${invitationId}`, { token: 'invited-profile-jwt' });
    assert.equal(invitation.status, 200);
    assert.deepEqual(await invitation.json(), { invitation: { state: 'eligible' } });
    assert.deepEqual(calls, [{ userId: 'invited-profile-id', accessToken: 'invited-profile-jwt', invitationId }]);
  } finally {
    await new Promise(resolve => server.close(resolve));
  }
});

test('the invitation workflow exposes eligible, expired, and resolved states but rejects invalid identifiers', async () => {
  const calls = [];
  const states = ['eligible', 'expired', 'resolved'];
  const workflow = createContractWorkflow(
    { url: 'https://project.supabase.co', publishableKey: 'sb_publishable_example' },
    () => ({
      rpc: async (name, args) => {
        calls.push({ name, args });
        return { data: [{ state: states.shift() }], error: null };
      }
    })
  );
  const invitationId = '11111111-1111-4111-8111-111111111111';
  assert.deepEqual(await workflow.getInvitation({ accessToken: 'invited-profile-jwt', invitationId }), { state: 'eligible' });
  assert.deepEqual(await workflow.getInvitation({ accessToken: 'invited-profile-jwt', invitationId }), { state: 'expired' });
  assert.deepEqual(await workflow.getInvitation({ accessToken: 'invited-profile-jwt', invitationId }), { state: 'resolved' });
  await assert.rejects(workflow.getInvitation({ accessToken: 'invited-profile-jwt', invitationId: 'not-an-invitation' }), /Invitation must be valid/);
  assert.deepEqual(calls, [
    { name: 'get_contract_invitation_acceptance_state', args: { target_invitation_id: invitationId } },
    { name: 'get_contract_invitation_acceptance_state', args: { target_invitation_id: invitationId } },
    { name: 'get_contract_invitation_acceptance_state', args: { target_invitation_id: invitationId } }
  ]);
});
...
test('the Contract detail workflow qualifies the Contract Party profile relationship for PostgREST', async () => {
  const selections = [];
  const workflow = createContractWorkflow(
    { url: 'https://project.supabase.co', publishableKey: 'sb_publishable_example' },
    () => ({
          ...
          const result = table === 'contracts'
            ? {
                data: {
                  ...
                  contract_parties: [{ profile_id: 'profile-id', profiles: { display_name: 'Profile' } }],
                  contract_versions: [{ version_number: 1, contract_sections: [] }]
                },
                error: null
              }
            : { data: [], error: null };
  ...
  await workflow.getDetail({ userId: 'profile-id', accessToken: 'party-jwt', contractId: 'contract-id' });

  assert.match(
    selections.find(({ table }) => table === 'contracts').selection,
    /profiles!contract_parties_profile_id_fkey\(display_name, email\)/
  );
});
