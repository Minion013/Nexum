...
**What to build:** Signed-in users can use the Discover view in People to search and browse eligible professional Profiles, then open enough public-in-app context to decide whether to establish a working connection. The directory is never a public marketplace and does not grant Workspace or Contract access.
...
- [ ] A profile result clearly communicates that viewing or discovering a person grants no Workspace or Contract access.
...
- The Discover view now describes the access boundary both before search and on each result, supports name/username/headline search wording, and renders accessible loading, empty, and error states without exposing Workspace or Contract data.
