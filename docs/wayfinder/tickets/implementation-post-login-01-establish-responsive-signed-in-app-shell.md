...
- `app-shell.js` now normalises the desktop sidebar, mobile drawer, bottom navigation, and avatar menu across signed-in pages. Drawer and profile controls expose labelled expanded state; closing the drawer by its control or keyboard dismissal restores the collapsed state. The canonical destinations are Dashboard, Contracts, People, Workspaces, and Settings.
