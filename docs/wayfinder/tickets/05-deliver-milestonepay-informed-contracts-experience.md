...
**Superseded detail implementation (2026-08-11):** The dense Contract Draft detail described above is no longer the retained Contract detail implementation. `/contracts/:id` is now the permanent authenticated Contract detail with Overview, Deliverables, and Chat, a project-change action, a milestone/payment rail, and a durable Profile-scoped simulated active Contract. This historical ticket remains evidence for the Contracts list and baseline access work only; it does not complete private evidence, criteria-gated review, or chain-authoritative payment requirements.
