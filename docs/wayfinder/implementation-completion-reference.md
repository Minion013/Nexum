...
The application-foundation decisions are tracked in the [PactFlow application foundation map](application-foundation-map.md). It supersedes the old role-based local-session model for all new work:
...
## 8 August 2026 post-login product update

- A user owns one Profile, may create and switch Workspaces, and manages their bio/avatar and linked wallet from Profile Settings. The sign-in entry safely distinguishes continuing with an active Profile from using a different account; the Supabase magic-link email is branded and testnet-safe.
- The signed-in hierarchy is **Workspace → Project → governing Contract**. A Contract can be listed from a Workspace, but a Project is the user-facing work container. Its new Proposal is a dedicated page and remains inside the owning Workspace until it is explicitly shared.
- Invitations begin with previous collaborators and permit a new exact-email recipient. They grant Project access only after acceptance and never imply Workspace membership.
- Resolution Authorities remain Contract/case-scoped operations. They have no ordinary post-login account type, navigation item, or dashboard surface.
- Profile wallet funds and the balance in each Project’s Escrow Vault are always separate amounts. [Define the testnet payment-method and funding model](tickets/implementation-testnet-20-define-the-testnet-payment-method-and-funding-model.md) selects the public MockEUSD faucet as the sole source of valueless demo funds; a Stripe test payment is not a real fiat-to-crypto bridge.
- A private in-app notification inbox is required for invitations and Project lifecycle actions. Email delivery does not replace it.

## 8 August 2026 update — private notification inbox foundation

- [x] Migrations `20260808120000_add_private_notifications.sql` and `20260808120500_order_private_notifications_by_time.sql` are applied to the linked Supabase project. They create an idempotent, private Profile inbox, a safe server-only read-state transition, and newest-first ordering.
- [x] The authenticated shell shows an unread notification count and `/notifications` renders the signed-in Profile's time-ordered inbox. Exact-email invitations, invitation acceptance, ready Contract Versions, and wallet-backed acceptances now emit durable, safe-link events for existing recipients.
- [x] The rollback-only linked RLS proof verifies an exact-email invitee receives and can mark only their own notification, while unrelated Profiles cannot read it. `supabase db lint --linked` passes; the local suite passes 45 tests, typecheck, and client build.
- [ ] Ticket 19 is **Partial — durable inbox foundation**, not testnet-MVP complete. Durable Project sources and the funding, evidence, review-window, dispute, amendment, and settlement workflows still need their matching idempotent events; browser end-to-end verification remains open.

## 8 August 2026 update — test-funding decision

- [x] Ticket 20 selects the public Base Sepolia `MockEUSD.faucet` as the sole MVP source of test funds. The Buyer signs a valueless, on-chain faucet transaction from their own linked wallet, refreshes that wallet balance, then separately approves and funds the Project Vault.
- [x] Stripe and every other payment provider are out of the MVP funding path. PactFlow creates no PaymentIntent, handles no card data or webhook, and runs no token-dispensing service. The UI must state that eUSD has no monetary value and that neither the faucet nor Base Sepolia test ETH is a payment, conversion, redemption, or custody flow.
- [x] Ticket 08 now defines its required browser verification: a wallet test double must prove the disclosure, faucet signature, balance refresh, and separate approve/fund signatures, while proving the absence of card/provider/PaymentIntent/webhook/server-dispense behaviour.
- [ ] Ticket 20's decision checklist is complete, but no browser faucet, Contract-specific Vault creation, buyer funding transaction, or chain-authoritative status view exists. Ticket 07 remains **Partial — deployed contract evidence**; Ticket 08 remains **Partial — contract-foundation evidence**; Ticket 09 remains blocked on those seams.
