...
export function NotificationsLoading() {
  return <section className="app-panel notification-inbox" aria-live="polite" aria-busy="true"><div className="notification-heading-row"><div><p className="eyebrow">Private inbox</p><h1>Notifications</h1></div><a className="notification-close" href="/home" aria-label="Close notifications and return to Dashboard" title="Close notifications"><span aria-hidden="true">×</span></a></div><p className="page-intro">Loading your private inbox...</p><p className="empty">Loading notifications...</p></section>;
}

export function NotificationsError({ message }: { message: string }) {
  return <><p className="notification-error" role="alert">{message}</p><section className="app-panel notification-inbox" aria-labelledby="notifications-error-title"><div className="notification-heading-row"><div><p className="eyebrow">Private inbox</p><h1 id="notifications-error-title">Notifications could not be loaded.</h1></div><a className="notification-close" href="/home" aria-label="Close notifications and return to Dashboard" title="Close notifications"><span aria-hidden="true">×</span></a></div><p className="page-intro">Your private inbox is unavailable right now. Please check your sign-in and try again.</p><p><a className="button primary" href="/home">Return to Dashboard</a></p></section></>;
}
...
export function NotificationsContent({ data, markingId, actionError, onMarkRead }: { data: NotificationsData; markingId: string | null; actionError: string; onMarkRead: (notification: NotificationEntry) => void }) {
  return <>
    <section className="notification-heading"><div className="notification-heading-row"><div><p className="eyebrow">Private inbox</p><h1>Notifications</h1></div><a className="notification-close" href="/home" aria-label="Close notifications and return to Dashboard" title="Close notifications"><span aria-hidden="true">×</span></a></div><p className="page-intro">Only activity relevant to your Profile, Contract Parties, and exact-email invitations appears here.</p></section>
