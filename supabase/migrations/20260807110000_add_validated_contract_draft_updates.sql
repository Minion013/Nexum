...
begin
  ...
  if not exists (
    ...
    where id = target_contract_id and status in ('private_draft', 'negotiation')
  ) then raise exception 'Only an unsigned Contract draft can be edited.'; end if;
  if jsonb_typeof(draft_scope) <> 'object'
    or char_length(trim(coalesce(draft_scope ->> 'title', ''))) not between 1 and 160
    or char_length(trim(coalesce(draft_scope ->> 'description', ''))) not between 1 and 4000 then
    raise exception 'A Contract title and scope are required.';
  end if;
  if jsonb_typeof(draft_milestones) <> 'array' or jsonb_array_length(draft_milestones) not between 2 and 3 then
  ...
  for milestone in select value from jsonb_array_elements(draft_milestones)
  loop
  ...
  limit 1;
  if latest_version_id is null then raise exception 'The Contract has no draft Version.'; end if;

  insert into public.contract_sections (contract_version_id, section_type, position, terms)
  values (latest_version_id, 'scope', 0, draft_scope)
  on conflict (contract_version_id, section_type)
  do update set terms = excluded.terms;
  insert into public.contract_sections (contract_version_id, section_type, position, terms)
  values (latest_version_id, 'milestones', 3, jsonb_build_object('items', draft_milestones))
  on conflict (contract_version_id, section_type)
  do update set terms = excluded.terms;
  insert into public.contract_sections (contract_version_id, section_type, position, terms)
  values (latest_version_id, 'payment', 4, jsonb_build_object(
    'settlement_token', 'eUSD testnet demonstration token',
    'total_allocation', total_allocation,
    'success_fee_bps', disclosed_success_fee_bps,
    'payment_authority', 'not_configured'
  ))
  on conflict (contract_version_id, section_type)
  do update set terms = excluded.terms;
...
revoke all on function public.update_contract_draft(uuid, jsonb, jsonb, bigint, integer) from public, anon;
grant execute on function public.update_contract_draft(uuid, jsonb, jsonb, bigint, integer) to authenticated;
