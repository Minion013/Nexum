...
async function refresh() { try { role = (await request('/api/session')).role; model = (await request('/api/agreement')).agreement; draftDirty = false; showError(); } catch { role = null; model = null; } render(); }
...
document.querySelectorAll('[data-role]').forEach(button => button.onclick = async () => { try { await request('/api/session', { method: 'POST', body: JSON.stringify({ role: button.dataset.role }) }); await refresh(); } catch (error) { showError(error.message); } });
